 ...
 * AI Use:
 * ---------
 * Claude was utilized to generate extractIPv4, main(), and the test cases.
 * I have labeled what code was created by me, and all else was generated by Claude.
 * 
 * See AI_Disclosure.md for full details on the use of AI in this project.
 */
...
int extractIPv4(const char* str, unsigned long* outAddress, int* outPort) {
    ...
    while (i < len) {
        ...
        if (isdigit((unsigned char)str[i]) && !prevIsChainChar) {
            ...
            // attempt to extract an IPv4 address (Heidi Schieber)
            for (int o = 0; o < 4 && success; o++) {
                int runLen = getDigitRun(str, pos);

                // runLen out of range (Heidi Schieber)
                ...
                // if value is too large, break (Heidi Schieber)
                if (value > 255) {
                    success = 0;
                    break;
                }
                ...
                // break if there are not four octets (Heidi Schieber)
                if (o < 3) {
            ...
            // if 4 in range octets were retrieved then the current IPv4 address is currently a success (Heidi Schieber)
            if (success) {
                int port = -1;

                // check if there is a valid port (Heidi Schieber)
                if (str[pos] == ':') {
                    int afterColon = pos + 1;
                    int runLen = getDigitRun(str, afterColon);

                    // check if the port number is valid (Heidi Schieber)
                ...
                // Lines 143-148 were created by Heidi Schieber
                else if (str[pos] == '.')
                {
                ...
                // if there is a success set outAdress and outPort then return (Heidi Schieber)
                if (success) {
...
// TestCase structure (Heidi Schieber)
typedef struct {
...
// comptues the address in decimal number (Heidi Schieber)
...
// function that runs tests (Heidi Schieber)
void runTests(void) {
    TestCase tests[] = {
    ...
    // tests all the test cases intialized above (Heidi Schieber)
    for (int t = 0; t < numTests; t++) {
        unsigned long address = 0;
        int port = 0;
        int result = extractIPv4(tests[t].input, &address, &port);

        // checks if the results from extractIPv4 mathc the expected return output (Heidi Schieber)
        int ok = (result == tests[t].expectedReturn);
        if (ok && result == 1) {
            ok = (address == tests[t].expectedAddress) && (port == tests[t].expectedPort);
        }

        // display the results of the test  (Heidi Schieber)
...
// main function (Heidi Schieber)
int main(int argc, char* argv[]) {
    // if the flag "--test" is provided then run the tests (Heidi Schieber)
    ...
    while (1) {
        ...
        // display the IPv4 address if it was able to be extracted and an invalid input message otherwise (Heidi Schieber)
